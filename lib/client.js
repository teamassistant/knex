...
class Client extends EventEmitter {
  ...
  parameterize(values, notSetValue, builder, bindingsHolder) {
    ...
    if (values.length === 1 && !!values[0] && values[0].forceType) {
      const whereInValues = Array.isArray(values[0].value) ? values[0].value : [values[0].value];
      values = whereInValues.map(value => { return { value, forceType: values[0].forceType } });
    }
    ...
    while (++i < values.length) {
      ...
      if (isPlainObject(value) && !(value.hasOwnProperty('value') && value.hasOwnProperty('forceType'))) {
  ...
  values(values, builder, bindingsHolder) {
    ...
    if (values && values.isRawInstance) {
      ...
      return this.values([values], builder, bindingsHolder);
    }
