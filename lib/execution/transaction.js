...
function makeTransactor(trx, connection, trxClient) {
  ...
  transactor.context.meta = function(value) {
    if (typeof value !== 'object') {
      throw new Error('Knex meta can only be extended with an Object!');
    }
    trx.__meta = { ...(trx.__meta || {}), ...value };
    connection.__meta = { ...(connection.__meta || {}), ...value };
    return transactor;
  };
  ...
  if (trx.client.transacting) {
    ...
    transactor.commit = async value => {
      const result = await trx.commit(connection, value);
      transactor.emit('committed', value);
      return result;
    }
  ...
  transactor.getTransactionId = () => trx.getTransactionId();
