...
class Client_MSSQL extends Client {
  ...
  _typeForBinding(binding) {
    ...
    if(typeof binding === "string" && binding[0] === "{" && binding[binding.length - 1] === "}") {
    ...
    switch (typeof binding) {
      ...
      default: {
        ...
        if (typeof binding === 'object' && !!binding && typeof binding.value !== 'undefined') {
          return [binding.value, Driver.TYPES[binding.forceType]];
        }
  ...
  _setReqInput(req, i, inputBinding) {
    ...
    req.addParameter(bindingName, tediousType, binding && binding.value || binding, options);
  }
