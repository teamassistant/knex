...
const varcharTypeColumns = require('../dialects/mssql/utilities/varcharTypeColumns');
const charTypeColumns = require('../dialects/mssql/utilities/charTypeColumns');
const {
...
class Builder extends EventEmitter {
  ...
  where(column, operator, value) {
    ...
    let forceType = false;
    if (this.client.config.client === 'mssql' && typeof column === 'string') {
      const [tableAlias, columnName] = column.split('.');
      const columnNameResolved = columnName || tableAlias;
      forceType = varcharTypeColumns.includes(columnNameResolved) ? 'VarChar' : forceType;
      forceType = charTypeColumns.includes(columnNameResolved) ? 'Char' : forceType;
    }
    ...
    this._statements.push({
      ...
      column,
      operator,
      value: forceType ? { value, forceType } : value,
      not: this._not(),
      bool: this._bool(),
  ...
  whereIn(column, values) {
    ...
    this._statements.push({
      ...
      type: 'whereIn',
      column,
      value: forceType ? { value: values, forceType } : values,
      not: this._not(),
      bool: this._bool(),
    });
    return this;
...
const validateWithArgs = function (alias, statement, method) {
  if (typeof alias !== 'string' && !alias.isRawInstance) {
